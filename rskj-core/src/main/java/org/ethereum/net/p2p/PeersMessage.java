...
public class PeersMessage extends P2pMessage {
    ...
    private Set<DecodedPeer> peers;

    public PeersMessage(byte[] payload) {
        super(payload);
    }

    public PeersMessage(Set<DecodedPeer> peers) {
        this.peers = peers;
        parsed = true;
    }
    ...
    private void parse() {
        ...
        for (int i = 1; i < paramsList.size(); ++i) {
            ...
            try {
                ...
                DecodedPeer peer = new DecodedPeer(address, peerPort, peerId);
                peers.add(peer);
    ...
    private void encode() {
        ...
        List<DecodedPeer> peerList = new ArrayList<>(this.peers);
    ...
    public Set<DecodedPeer> getPeers() {
        if (!parsed) {
            this.parse();
        }
        return peers;
    ...
    public String toString() {
        ...
        for (DecodedPeer peerData : peers) {
            sb.append("\n       ").append(peerData);
        }
